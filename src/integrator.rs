...
impl<F: FnMut(f64) -> f64> Integrator<F> {
    ...
    pub fn step(&mut self) -> f64 {
        ...
        *steps += 1;
        ...
        let new_res = (self.a(0)
            + self.a(half) / (1. - f64::from(total).powi(2))
            + 2. * (1..half)
                .map(|k| self.a(k) / (1. - f64::from(k << 1).powi(2)))
                .sum::<f64>())
            / f64::from(half);
        let Self {
            ref mut err,
    ...
    pub fn with_prec(&mut self, prec: f64) -> f64 {
        while self.err.is_nan() || self.err > prec {
            self.step();
        }
        self.last
    }
        ...
        let res = self.cached[1]
            ...
            + (1u16..half)
                .map(|n| {
                    ...
                    self.cached[2 + idx]
                        * (PI * f64::from(k) * f64::from(n) / f64::from(half)).cos()
                })
                .sum::<f64>();
        res * 2.
    }
...
pub fn numerator_idx(mut nmtr: u16, mut total: u16) -> usize {
    while nmtr & 1 == 0 {
        nmtr >>= 1;
        total >>= 1;
    }
    (total as usize >> 1) - 1 + (nmtr >> 1) as usize
}
